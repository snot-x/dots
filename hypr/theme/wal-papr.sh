#Strongly based on adi1090x's pywal script, but I wanted to use a specific wallpaper image rather than a randomly chosen image from a defined wallpapers folder. I found myself running the script twenty plus times before getting an image I wanted at the time, often getting the same images multiple times.
#So this is my attempt: (github.com/snot-x))
...
## Pywal Theme
source_pywal() {
    local wallpaper="/home/snot/Pictures/gh-walls1/0296.jpg"

    # Check if the wallpaper file exists
    if [[ ! -f "$wallpaper" ]]; then
        notify-send -h string:x-canonical-private-synchronous:sys-notify-noimg -u low -i ${PATH_MAKO}/icons/picture.png "Wallpaper not found: $wallpaper"
        exit 1
    fi
...
if [[ "$1" == '--default' ]]; then
    source_default
elif [[ "$1" == '--pywal' && -n "$2" ]]; then
    source_pywal "$2"
else
    echo "Available Options: --default  --pywal <wallpaper_path>"
    exit 1
fi
